...
//	showhide("box3", "hide");
	...
	if (document.getElementById("invest")) {		// id of div
		...
		$("#addrow").on("click", function () {
			...
			var cols = "";
			counter++;
			cols += '<td><input id="fundid'+counter+'" name="fundid'+counter+'" type="hidden" value=""/></td>';
			cols += '<td><select type="text" class="form-control" data-row="'+counter+'" id="fundname'+counter+'" name="fundname'+counter+'  placeholder="select">';
			cols += fundselect + '</select></td>';
			cols += '<td><input type="text" class="form-control" id="riskrate'+counter+'" name="riskrate'+counter+'" disabled /></td>';
			cols += '<td><input type="text" class="form-control" id="fundtype'+counter+'" name="fundtype'+counter+'" disabled /></td>';
			cols += '<td><input type="text" class="form-control" id="mininvest'+counter+'" name="mininvest'+counter+'" disabled /></td>';
			cols += '<td><input type="text" class="form-control" data-row="'+counter+'" id="perc'+counter+'" name="perc'+counter+'"/></td>';
			cols += '<td><input type="text" class="form-control" id="indamt'+counter+'" name="indamt'+counter+'" disabled /></td>';
			cols += '<td class="col-xs-12 col-sm-1"><a id="kiid'+counter+'" href=""  target="_blank" style="display: none"><i class="fa fa-file-pdf-o fa-2x" aria-hidden="true"></i></a></td>';
			cols += '<td class="col-xs-12 col-sm-1"><a id="fsheet'+counter+'" href="" target="_blank" style="display: none"><i class="fa fa-file-pdf-o fa-2x" aria-hidden="true"></i></a></td>';

			cols += '<td><input id="'+counter+'" type="button" class="ibtnDel btn btn-md btn-danger "  value="Delete"></td>';
			newRow.append(cols);
...
//			alert(this.dataset.row);
			...
			for(var i=0; i<funds.length; i++) {
				if(funds[i].fund_id == this.value) {
					...
					document.getElementById("mininvest"+c).value = '3750.00';
...
function displayquestion(j) {
	jQuery(function($){
		...
		for(k=0; k<categories.length;k++) {
			str = "<div id='cat'"+k+" class='col-xs-12 fade-in'><center><h3><b>"+" "+categories[k].rcats_name+"</b></h3></center>";
			for(var j=0; j<questions.length; j++) {
				if(categories[k].rcats_catid == questions[j].rque_categoryid) {
					str += '<h5">'+" "+questions[j].rque_question+'</h5>'
					selected=false;
					str += '<table class="mychoices">';
			...
			if(c==categories.length) {
				str += '<button class="continueBtn" onclick="showNextCat('+c+')">Finish</button>';				
			} else {
				str += '<button class="continueBtn" onclick="showNextCat('+c+')">Continue</button>';
			}
			if(c>1) {
				str += '<button class="backBtn" onclick="showPrevCat('+c+')">Back</button>';
			}
			str += '<hr></div>';
			catstr.push(str);
			c++;
...
/*function listAnswersToConsole() {		//tmp for debugging
	for(var i=0; i<answers.length; i++) {
		console.log(answers[i].resp_question+" "+answers[i].resp_answer);
	}
}*/
...
console.log("numberofanswers is "+numberofanswers);
	if(c==categories.length && numberofanswers != numberofquestions) {
		showNextCat(0);
	} else {
		jQuery(function($){
			if(!catstr[c]) {
