...
class Communication
{
    ...
    static function SendEmail($_account,$_receiver,$_replyto,$_bodyText,$_bodyHTML,$_subject="",$_test=false,$_attachments=null,$_fakeSender = "",$_senderName="")
    {
        if($_account == null)
...
class TicketMessage extends Action
{
	...
	function Save($_ticketId,$_overwrite=false,$_time=null,$_ticket=null)
	{
        ...
        if(!$result && $errorCode == 1366)
            $result = DBManager::Execute(true, $do . " INTO `" . DB_PREFIX . DATABASE_TICKET_MESSAGES . "` (`id` ,`time` ,`created` ,`ticket_id` ,`text`, `html`,`fullname` ,`email` ,`company` ,`ip`, `phone` ,`call_me_back`,`country`,`type`,`sender_id`,`channel_id`,`hash`,`subject`) VALUES ('" . DBManager::RealEscape($this->Id) . "', " . DBManager::RealEscape($time) . "," . DBManager::RealEscape($this->Created) . ", '" . DBManager::RealEscape($_ticketId) . "', '" . DBManager::RealEscape(DBManager::EncodeField($this->Text)) . "', '" . DBManager::RealEscape(DBManager::EncodeField($this->HTML)) . "','" . DBManager::RealEscape(DBManager::EncodeField($this->Fullname)) . "', '" . DBManager::RealEscape($this->Email) . "', '" . DBManager::RealEscape(DBManager::EncodeField($this->Company)) . "', '" . DBManager::RealEscape($this->IP) . "', '" . DBManager::RealEscape($this->Phone) . "', " . (($this->CallMeBack) ? 1 : 0) . ", '" . DBManager::RealEscape($this->Country) . "', '" . DBManager::RealEscape($this->Type) . "', '" . DBManager::RealEscape($this->SenderUserId) . "', '" . DBManager::RealEscape($this->ChannelId) . "', '" . DBManager::RealEscape($this->Hash) . "', '" . DBManager::RealEscape(DBManager::EncodeField($this->Subject)) . "');", $errorCode);
...
class Ticket extends Action
{
    ...
    function SendOperatorReply($_messageId, $_email, $_qMessageId="")
    {
        ...
        if($mailbox != null)
        {
            ...
            Communication::SendEmail($mailbox,str_replace(array(",,",";"),",",$_email),$mailbox->Email,$tsData[0],$tsData[1],Mailbox::GetSubject($reply->Subject,$_email,$this->Messages[0]->Fullname,$this->Group,"",$this->Messages[0]->Company,$this->Messages[0]->Phone,$this->Messages[0]->IP,$this->Messages[0]->Text,Server::$Groups[$this->Group]->GetDescription($this->Language),$this->Messages[0]->Customs),false,$reply->Attachments,"",$senderName);
        }
    }
    ...
    static function Exists($_hash, &$id, &$group, &$language)
    {
        ...
        //INNER JOIN `" . DB_PREFIX . DATABASE_TICKET_MESSAGES . "` AS `dbm` ON `dbt`.`id`=`dbm`.`ticket_id`
        //deleted = 0;
