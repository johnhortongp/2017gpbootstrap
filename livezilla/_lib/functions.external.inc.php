...
class OverlayChat
{
    ...
    function InitFeedback($_userInitiated=true)
    {
        ...
        if($_userInitiated || !empty($cid))
            if($_userInitiated || Feedback::GetByChatId($cid)==null)
                if($this->Version == 2)
                    VisitorMonitoring::$Visitor->AddFunctionCall("OverlayChatWidgetV2.ToggleFeedback(true);",false);
    }
    ...
    function GetEyeCatcherV2(){
        ...
        if(!empty($dcp))
        {
            ...
            if(!isset($_GET["echst"]))
                VisitorMonitoring::$Visitor->AddFunctionCall("lz_ec_sub_header_p='".base64_encode(str_replace("<!--operator_name-->",Server::$Operators[$dcp]->Fullname,LocalizationManager::$TranslationStrings["client_ec_sub_text_personal"]))."';",false);
