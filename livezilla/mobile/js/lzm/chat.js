...
function SelectView(id,required) {
    ...
    if (true/*id != lzm_chatDisplay.selected_view || required*/)
    {
...
function initSaveTicketDetails(ticket, channel, status, group, editor, language, name, email, company, phone, message, attachments, comments, customFields, subStatus, subChannel, _chat, mc, subject, rem_time, rem_status, priority, feedback) {
...
function uploadSaveTicketDetails(_action, _chat, _feedback) {

    if(_feedback != null)
        CommunicationEngine.FeedbacksUpdateTimestamp = '';

    _action = (d(_action)) ? _action : 'save-details';
    this.CommunicationEngine.pollServerTicket(lzm_commonTools.clone(this.CommunicationEngine.uploadTickets), [], _action, _chat, _feedback);
...
function scrollToEmail(no){
    if(no > 0)
        $('#incoming-email-list-active').scrollTop(parseInt(no) * 22);
}
