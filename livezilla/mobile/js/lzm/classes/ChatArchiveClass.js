...
ChatArchiveClass.prototype.CreateArchiveListLine = function(aChat, selectedChatId, inDialog, elementId) {
    ...
    else // mobile
    {
        ...
        if(true) //if(LocalConfiguration.UIShowAvatars)
        {
            ...
            archiveLineHtml += '<div>' + avatar + '</div><div class="mobile-archive-line-user-name" style="left: 80px;">' + lzm_commonTools.SubStr(name,23,true) + '</div></div>';
        }
        else
            archiveLineHtml += '<div class="mobile-archive-line-user"><div class="mobile-archive-line-user-name">' + lzm_commonTools.SubStr(name,23,true) + '</div></div>';

        if(question != '-')
            archiveLineHtml += '<div class="mobile-archive-line-message">' + statusIcon + question + '</div>';
        if(email != '-')
...
ChatArchiveClass.prototype.createArchiveContextMenu = function(myObject) {
    var disabledClass,cpId = '', qId = '', contextMenuHtml = '';
    if (myObject.t == 0)
    {
        qId = myObject.iid;
    }
    ...
    contextMenuHtml += '<div onclick="showArchivedChat(\'' + qId + '\',\'' + myObject.cid + '\');">' + '<span id="archive-show-chats" class="cm-line cm-click">' +t('All Chats of this User') + '</span></div><hr />';
    disabledClass = (myObject.t == 0 || myObject.t == 2) ? ' class="ui-disabled"' : '';
    contextMenuHtml += '<div' + disabledClass + ' onclick="sendChatTranscriptTo(\'' + myObject.cid + '\');"><span id="archive-send-transcript" class="cm-line cm-click">' + t('Send transcript to...') + '</span></div>';
    ...
    disabledClass = (myObject.t == 0 || myObject.t == 2) ? ' class="ui-disabled"' : '';
    contextMenuHtml += '<div' + disabledClass + ' onclick="removeArchiveListContextMenu();Chat.CreateTicket(\'' + myObject.cid + '\');"><span id="archive-create-ticket" class="cm-line cm-click">' + t('Create Ticket') + '</span></div>';
