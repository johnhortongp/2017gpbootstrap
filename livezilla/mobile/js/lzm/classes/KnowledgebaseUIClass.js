...
KnowledgebaseUI.prototype.CreateSearchResultLine = function(resource, searchString, _context, inDialog) {
    ...
    //content = content.replace(regExp, '<span class="search-highlight">' + searchString + '</span>');
...
KnowledgebaseUI.ChatKBAutoSearch = function(_instant){
    ...
    var title,i,previewHeight,shortcut,row,typingNow = lzm_chatTimeStamp.getServerTimeString(null, false, 1);
        ...
        if (editorContents.length > 1)
        {
            ...
            if ($('#chat-progress').height() > 200 && frequentlyUsedResources.length > 0)
            {
                ...
                for (i=0; i<maxIterate; i++)
                {
                    ...
                    row = '<tr class="lzm-unselectable" style="cursor: pointer;" onclick="useEditorQrdPreview(\'' + frequentlyUsedResources[i].rid + '\');">' + resourceText + '</tr>';
...
KnowledgebaseUI.ShowEntry = function (_entryId, _entryType, _ticketReplyWindowId, _sendToChat){

    removeKBContextMenu();

    _entryType = d(_entryType) ? _entryType : KnowledgebaseUI.TypeText;
    _sendToChat = d(_sendToChat) ? _sendToChat : null;
    ...
    function __GetSettingsContent(){
        ...
        html += lzm_inputControls.createInput('qrd-knb-id-text', '', lzm_commonTools.htmlEntities(kbEntryObject.rid), t('ID:'), '', 'text', '') + '</fieldset>';
        ...
        html += '<fieldset class="lzm-fieldset" id="qrd-knb-search-fs"><legend>' + t('Search') + '</legend><div' + fulltextDisabled + '>';
        ...
        html += lzm_inputControls.createInput('qrd-knb-shortcuts-text', shortcutDisabeld, lzm_commonTools.htmlEntities(kbEntryObject.s), t('Shortcut (word, global)'), '<span class="icon-blue" style="font-weight:bold;">/</span>', 'text', 'a');
        ...
        html += lzm_inputControls.createInput('qrd-knb-language-text', '', lzm_commonTools.htmlEntities(kbEntryObject.l), t('Language (leave blank for all)'), '', 'text', '');
