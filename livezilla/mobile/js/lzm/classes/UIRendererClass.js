...
UIRendererClass.prototype.resizeVisitorDetails = function() {
    ...
    $('.dialog-visitor-info, .embedded-visitor-info').each(function()
    {
        ...
        var lowerFieldsetHeight = contentHeight - upperFieldsetHeight;
        ...
        if(true || !lzm_chatDisplay.ticketDisplay.isFullscreenMode())
        {
        ...
        {
            // show preview
            //$('#matching-tickets-'+elementId+'-inner').css({overflow:'auto','height': upperFieldsetHeight + 'px'});
            //$('#ticket-content-'+elementId+'-inner').css({'box-sizing':'border-box',overflow:'auto',height: (lowerFieldsetHeight) + 'px'});
        }

        $('#matching-chats-'+elementId+'-inner').css({overflow:'auto','height': upperFieldsetHeight + 'px'});
        $('#chat-content-'+elementId+'-inner').css({overflow:'auto',height: (lowerFieldsetHeight) + 'px'});
...
UIRendererClass.prototype.resizeOperatorForwardSelection = function() {
    ...
    {
        var fwdTextHeight = Math.max((lzm_chatDisplay.dialogWindowHeight - 240), 100);
...
UIRendererClass.prototype.resizeMychats = function() {
    if ($('#chat-controls').length)
    {
        ...
        cib.css({left:(ccw-chatInfoWidth)+'px',top:'0'});

        if(cib.data('hidden')=='1' || ccw < CommonUIClass.MinWidthChatVisitorInfo)
        {
