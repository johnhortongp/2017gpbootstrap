...
LzmCustomInputs.prototype.getInputHtmlRow = function(inp) {
    ...
    return '<tr id="inp_row_'+inp.id+'">' +
        ...
        '<td>'+lzm_inputControls.createInput('in_' + inp.id, '', lzm_commonTools.htmlEntities(inp.name), '', '', 'text', '')+'</td>' +
        ...
        '<td>'+lzm_inputControls.createInput('icap_' + inp.id, '', lzm_commonTools.htmlEntities(inp.title), '', '', 'text', '')+'</td>' +
...
Chat.CreateTicket = function(_chatId, _createIfNotFound){
    ...
    if(chatObj != null)
    {
        ...
        {
            lzm_commonDialog.createAlertDialog(tid('related_ticket_exists').replace('<!--tid-->',chatObj.tid), [{id: 'yes', name: tid('yes')}, {id: 'no', name: tid('ignore')}]);
            $('#alert-btn-yes').click(function() {
                lzm_commonDialog.removeAlertDialog();
                SelectView('tickets');
                $('#search-ticket').val(chatObj.tid);
                var e = $.Event('keyup');
                e.which = 13;
...
VisitorManager.GetVisitor = function(_id){
    if(d(_id) && _id != null)
    {
        ...
        existing = lzm_commonTools.GetElementByProperty(VisitorManager.LoadedFullDataUsers,'id',_id);
        if(existing.length)
            return existing[0];
    }
    return null;
