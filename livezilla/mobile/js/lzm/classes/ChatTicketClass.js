...
ChatTicketClass.prototype.ShowTicket = function(_ticket, isNew, email, _chat, _feedback){
    ...
    if(!isNew && fullScreenMode && _ticket.messages[0].ui.length)
        footerString += lzm_inputControls.createButton('visitor-history', 'ticket-buttons' + disabledButtonClass, 'ChatVisitorClass.ShowVisitorInformation(\''+_ticket.messages[0].ui+'\',6,\'\',\''+_ticket.id+'\');', tid('visitor') + ' ' + tid('history'), '<i class="fa fa-history"></i>', 'lr', {'margin-left': '4px'}, '', 20, 'e');
...
ChatTicketClass.prototype.SaveTicket = function(_ticket, _isNew, _email, _chat, _feedback){
    ...
    else
    {
        ...
        if (_email.id == '' && _chat.cid == '' && _feedback == null)
        {
            ...
            uploadSaveTicketDetails(_isNew ? 'new-ticket' : 'save-details',{cid: ''});
        }
        else if (_chat.cid != '' || _feedback != null)
        {
            ...
            uploadSaveTicketDetails(_isNew ? 'new-ticket' : 'save-details', _chat, _feedback);
        }
        else
        {
            // ticket from email
...
ChatTicketClass.prototype.ShowMessageReplyPreview = function(ticket, messageNo, message, signature, comment, attachments, salutation, group, menuEntry) {
    ...
    {
        if (ticket.messages[i].em != '' && (i==messageNo || (d(ticket.messages[messageNo]) && ticket.messages[messageNo].t == '1') || email==''))
        {
...
ChatTicketClass.prototype.createTicketMessageAddLine = function(type,key,mkey,ticket,object,linecol,onclickAction,oncontextAction) {
    ...
    if(type=="comment")
    {
        ...
        lineHtml += '<tr class="message-line message-comment-line comment-line-' + ticket.id + '_' + mkey + ' lzm-unselectable" id="message-line-comment-' + ticket.id + '_' + key + '" '+onclickAction+oncontextAction+'>' +
            ...
            '<table class="comment-box" style="background:#fff !important;border: 1px solid '+opcol+'"><tr><td style="background:#fff !important;color:#666 !important;">' + lzm_inputControls.createAvatarField('avatar-box-small','',opid) + '</td>'+
            '<td style="background:#fff !important;color:#666 !important;"><div class="text-s" style="color:#777 !important;padding-bottom:3px;">'+messageTimeHuman+'</div><span style="background:#fff !important;color:#666 !important;">' + lzm_commonTools.htmlEntities(opname) + ': </span>' + ChatTicketClass.GetCommentText(object) + '</td></tr></table>' +
            '</td></tr>';
    }
    else if(type=="log")
    {
...
ChatTicketClass.__ShowTicket = function(ticketId, fromContext, emailId, chatId, _feedbackId, dialogId) {
    ...
    else
    {
        ...
        lzm_chatDisplay.ticketDialogId[ticketId] = lzm_chatDisplay.ticketDisplay.ShowTicket(lzm_chatDisplay.ticketDisplay.GetTicketById(ticketId,true), isNew, email, chat, feedback, dialogId);
    }
};
