...
ChatVisitorClass.prototype.UpdateVisitorInformation = function(_visitor) {
    try
    {
        var that = this;
        if(_visitor != null)
        {
            $(['d','e']).each(function()
            {
                var elementId = $(this)[0].toString() + '-' + _visitor.id;
                that.CreateHistoryTabControl(_visitor,elementId,$(this)[0].toString()=='d');

                if($('#visitor-details-'+elementId+'-list').length)
                {
                    $('#visitor-details-'+elementId+'-list').html(that.CreateVisitorInformationTable(_visitor,elementId)).trigger('create');
                    $('#visitor-history-'+elementId+'-placeholder-content-0').html(that.CreateBrowserHistory(_visitor,elementId)).trigger('create');

                    if(d(_visitor.rv))
                        for (var i=0; i<_visitor.rv.length; i++)
                        {
                            var recentHistoryHtml = that.CreateBrowserHistory(_visitor, elementId, _visitor.rv[i]);
                            $('#recent-history-'+elementId+'-' + _visitor.rv[i].id).replaceWith(recentHistoryHtml);
                        }

                    $('#visitor-comment-'+elementId+'-list').html(that.createVisitorCommentTable(_visitor, elementId)).trigger('create');
                    $('#visitor-invitation-'+elementId+'-list').html(that.createVisitorInvitationTable(_visitor, elementId)).trigger('create');

                    that.updateCoBrowsingTab(_visitor, elementId);

                    var numberOfHistories = (d(_visitor.rv) && !d(_visitor.IsOffline)) ? _visitor.rv.length + 1 : ((d(_visitor.rv)) ? _visitor.rv.length : ((!d(_visitor.IsOffline)) ? 1 : 0));
                    var numberOfComments = (d(_visitor.c)) ? _visitor.c.length : 0;
                    var numberOfInvites = (d(_visitor.r)) ? _visitor.r.length : 0;
                    var numberOfChats = (d(_visitor.ArchivedChats)) ? _visitor.ArchivedChats.length : 0;
                    var numberOfTickets = (d(_visitor.ArchivedTickets)) ? _visitor.ArchivedTickets.length : 0;
                    var numberOfFeedbacks = (d(_visitor.ArchivedFeedbacks)) ? _visitor.ArchivedFeedbacks.length : 0;
                    ...
                    if(d(_visitor.IsOffline) || d(_visitor.IsInternal))
                    {
                    ...
                    if(d(_visitor.IsInternal) || numberOfHistories==0)
                    {
                    ...
                    if(numberOfChats > 0 && !$('#visitor-info-'+elementId+'-placeholder-content-5').html().length)
                    {
                        ...
                        var chatsHtml = lzm_chatDisplay.archiveDisplay.CreateMatchingChats(_visitor,_visitor.ArchivedChats,elementId) + '<fieldset class="lzm-fieldset" style="margin:0;padding:0;" id="chat-content-'+elementId+'-inner"></fieldset>';
                        ...
                        if(d(_visitor.SelectedChatId))
                            $('#dialog-archive-list-'+elementId+'-line-' + _visitor.SelectedChatId).click();
                    }
                    ...
                    if(numberOfTickets > 0 && !$('#visitor-info-'+elementId+'-placeholder-content-6').html().length)
                    {
                        ...
                        var ticketsHtml = lzm_chatDisplay.ticketDisplay.createMatchingTickets(_visitor,elementId) + '<fieldset class="lzm-fieldset" style="margin:0;" id="ticket-content-'+elementId+'-inner"></fieldset>';
                        ...
                        if(lzm_chatDisplay.ticketDisplay.isFullscreenMode() && $('#visitor-info-'+elementId+'-placeholder-tab-6').hasClass('lzm-tabs-selected'))
                        {
                            ...
                            if(d(_visitor.SelectedTicketId))
                                $('#matching-ticket-list-'+elementId+'-row-' + _visitor.SelectedTicketId).click();
                        }
                    }

                    if(numberOfFeedbacks > 0 && !$('#visitor-info-'+elementId+'-placeholder-content-7').html().length)
                    {
                        $('#visitor-info-'+elementId+'-placeholder-tab-7').removeClass('ui-disabled');
                        var feedbacksHtml = lzm_chatDisplay.FeedbacksViewer.CreateMatchingFeedbacks(_visitor.ArchivedFeedbacks,elementId) + '<fieldset class="lzm-fieldset" style="margin:0;" id="feedback-content-'+elementId+'-inner"></fieldset>';
                    ...
                    $('#visitor-details-'+elementId+'-list').data('visitor', lzm_commonTools.clone(_visitor));
                }
            });
...
ChatVisitorClass.prototype.CreateVisitorInformationTable = function(_visitorObj, elementId) {
    ...
    if (_visitorObj.is_active)
    {
        ...
        //var hc = (visitorName.length) ? '<span class="text-blue text-xl">' + visitorName + '</span>' : '';
    ...
    {
        var visitorStatus = t('<!--status_style_begin-->Offline<!--status_style_end-->',[['<!--status_style_begin-->','<span class="text-orange">'],['<!--status_style_end-->','</span>']]);
    ...
    for (var myKey in visitorInfoArray)
    {
        if (visitorInfoArray.hasOwnProperty(myKey))
        {
            ...
            for (var k=0; k<visitorInfoArray[myKey].rows.length; k++)
            {
                ...
                var cssClass = (d(visitorInfoArray[myKey].rows[k].class)) ? ' ' + visitorInfoArray[myKey].rows[k].class : '';
                visitorInfoHtml += '<tr>';
                ...
                visitorInfoHtml += '<td class="key'+cssClass+'">' + visitorInfoArray[myKey].rows[k].title + '</td>';
                visitorInfoHtml += '<td class="content'+cssClass+'">' + contentString + '</td>';

                    if(visitorInfoArray[myKey].rows[k].editable)
                        visitorInfoHtml += '<td class="edit'+cssClass+'"><a href="#" onclick="EditVisitorDetails(\''+_visitorObj.id+'\',\''+visitorInfoArray[myKey].rows[k].editkey+'\',\''+elementId+'\');"><i class="fa fa-pencil icon-blue lzm-clickable2" style="float:right;"></i></a></td></tr>';
                    else
...
ChatVisitorClass.prototype.createVisitorInvitation = function(visitor) {
    ...
    return '<fieldset id="user-invite-form" class="lzm-fieldset">' +
...
ChatVisitorClass.ShowVisitorInformation = function (_visitorId, _activeTab, _selectedChatId, _selectedTicketId, _dialog) {
    ...
    var _visitorObj = VisitorManager.GetVisitor(_visitorId);
    if(_visitorObj == null)
    {
        _visitorObj = {id:_visitorId,unique_name:'',IsOffline:true,is_vi_ui_update:true};
    ...
    var visitorName = _visitorObj.IsOffline ? _visitorObj.unique_name : VisitorManager.GetVisitorName(_visitorObj);
    ...
    if(_dialog)
    {
        var dialogid = lzm_commonDialog.CreateDialogWindow(headerString, bodyString, footerString, 'user', winId, winId, 'cancel-visitorinfo',true, dialogData);
        $('#visitor-information').data('dialog-id', dialogid);
    }

    var detailsHtml = '<div id="visitor-details-'+elementId+'-list" style="overflow-y: auto;">' + lzm_chatDisplay.VisitorsUI.CreateVisitorInformationTable(_visitorObj, elementId) + '</div>';
    var historyHtml = '<div id="visitor-history-'+elementId+'-list"><div id="visitor-history-'+elementId+'-placeholder"></div></div>';
    var commentsHtml = '<div id="visitor-comment-'+elementId+'-list">' + lzm_chatDisplay.VisitorsUI.createVisitorCommentTable(_visitorObj, elementId) + '</div>';
    var invitationsHtml = '<div id="visitor-invitation-'+elementId+'-list">' + lzm_chatDisplay.VisitorsUI.createVisitorInvitationTable(_visitorObj, elementId) + '</div>';
    ...
    if(!d(_visitorObj.rv))
    {
        ...
        if(!lzm_commonTools.GetElementByProperty(VisitorManager.LoadedFullDataUsers,'id',_visitorObj.id).length)
        {
            _visitorObj.ArchivedChats = [];
            _visitorObj.ArchivedTickets = [];
            VisitorManager.LoadedFullDataUsers.push(_visitorObj);
        }
        CommunicationEngine.InstantPoll(true);
    }
    else
        lzm_chatDisplay.VisitorsUI.UpdateVisitorInformation(_visitorObj,false);
};
